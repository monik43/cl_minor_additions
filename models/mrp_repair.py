    ...
    def _get_lot_id(self):
        for rec in self:
            if not rec.lot_id and rec.x_ticket:
                ...
                elif not rec.ticket_x.x_lot_id and rec.ticket_x.x_sn and self.env['stock.production.lot'].search([('name','=',rec.ticket_x.x_sn[:4].upper()),('product_id', '=', rec.product_id.id)]):
                    print("ticket_x no tiene x_lot_id pero lo ha encontrado")
